...
CI/CD workflow (`.github/workflows/deploy_prod.yaml`) при пуше в ветку `main` автоматически разворачивает проект на заранее настроенном сервере — подключение производится через Secrets и Variables репозитория (например: `HOST`, `SERVER_USER`, `SSH_KEY`, `PORT`, и др.). После успешного деплоя приложение можно протестировать по адресу:

```
http://<HOST>:<PORT>/
```

Замените `<HOST>` и `<PORT>` на значения, использующиеся в вашем репозитории (секреты/переменные). Для дополнительной проверки можно смотреть логи контейнеров и состояния сервисов на удалённом сервере.

Важные моменты, если вы хотите воспользоваться данным CI/CD workflow и развернуть проект на своем удаленном сервере!!
- HOST - это IP аддресс вашего сервера
- SERVER_USER - имя юзера (например root)
- SSH_KEY - значение ссш ключа (приватного), который относится к авторизированным на Вашем удаленном сервере
- у пользователя гитхаб с данным ключом должен быть доступ к данному проекту(он публичный, так что должен быть)
- предзапуском CI/CD необходимо склонировать данный репозиторий из гитхаб в папку /home на Вашем удаленном сервере (если вы выберите другую - исправьте в deploy-prod.yaml)
- также на вашем удаленном сервере должен быть установлен docker, docker-compose

### Развернуто: https://crmcrudapp.midnight-hunter.org
(Приложение доступно для ознакомления по этому адресу; 
OpenAPI: https://crmcrudapp.midnight-hunter.org/docs)